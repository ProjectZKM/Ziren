...
fn bench_whir_vs_basefold() {
    ...
    for &n in &[18usize, 20] {
        let ff = 4usize;
        let n_rounds = (n - 2) / ff;
        let queries: Vec<usize> = (0..n_rounds).map(|i| 124usize >> i).collect();
        ...
        let whir_cfg = whir_config_for(n, ff, &queries, 2, 2, 16);
...
fn jagged_whir_roundtrip_matches_basefold_layout() {
    ...
    assert_eq!(
        verify_jagged_whir_rounds(
            ...
            cfg.clone(),
            lsh as u32,
            std::slice::from_ref(&commit.original_commitment),
            &[commit.area],
            &point,
            &proof,
            evaluation_claim,
...
/// The ring commit stacks the dense polynomial once and commits it under WHIR
/// straight from the interleaves: its packing, commitment and stripes are those
/// of the standalone jagged-WHIR commit over the same materialized dense, and
/// the interleaves it keeps for the reduction reproduce that dense.
#[test]
fn ring_whir_commit_matches_standalone_commit() {
    use crate::basefold::stacked::dense_from_interleaved_mles;
    use crate::config::BasefoldRing;
    use crate::jagged::{compute_jagged_metadata, materialize_dense_jagged};
    use crate::jagged_pcs::jagged::views_over_traces;
    use crate::kb31_poseidon2::koala_bear_poseidon2::KoalaBearPoseidon2;
    use crate::whir::jagged::{commit_jagged_whir_generic, core_whir_config};

    type F = InnerVal;
    let lsh = crate::jagged_pcs::DEFAULT_LOG_STACKING_HEIGHT as usize;
    let mut rng = StdRng::seed_from_u64(0x51A6);
    let traces: Vec<(String, RowMajorMatrix<F>)> =
        [(3usize, 1usize << 17), (7, 1 << 16), (12, 4096)]
            .iter()
            .enumerate()
            .map(|(i, (w, h))| {
                let vals: Vec<F> = (0..w * h).map(|_| rand_kb(&mut rng)).collect();
                (format!("chip{i}"), RowMajorMatrix::new(vals, *w))
            })
            .collect();
    let views = views_over_traces(traces);

    let ring = <KoalaBearPoseidon2 as BasefoldRing>::commit_multilinears(&views, None);
    let packing = compute_jagged_metadata::<F>(&views);
    assert_eq!(ring.packing.dense_len, packing.dense_len);
    assert_eq!(ring.packing.offsets, packing.offsets);
    assert_eq!(ring.packing.total_values, packing.total_values);

    let dense = materialize_dense_jagged::<F>(&views, packing.dense_len);
    let (standalone, standalone_data) = commit_jagged_whir_generic(
        vec![(String::from("<jagged-dense>"), RowMajorMatrix::new(dense.clone(), 1))],
        build_mmcs(),
        Arc::new(Radix2DitParallel::<F>::default()),
        core_whir_config(lsh),
    );
    assert_eq!(ring.commit.original_commitment, standalone.original_commitment);
    assert_eq!(ring.commit.area, standalone.area);
    let ring_whir = ring.whir_data.as_ref().expect("the inner ring commits under WHIR");
    assert_eq!(ring_whir.stacked_data.commitment, standalone_data.stacked_data.commitment);
    assert_eq!(ring_whir.stacked_data.stripes.len(), standalone_data.stacked_data.stripes.len());
    for (a, b) in
        ring_whir.stacked_data.stripes.iter().zip(standalone_data.stacked_data.stripes.iter())
    {
        assert_eq!(a.guts().as_slice(), b.guts().as_slice());
    }
    let rebuilt = dense_from_interleaved_mles::<F>(
        &ring.prover_data.stacked_data.interleaved_mles,
        ring.prover_data.area,
    );
    assert_eq!(rebuilt[..dense.len()], dense[..]);
    assert!(ring.prover_data.stacked_data.pcs_batch_data.encoded_codewords.is_empty());
}

/// The tiled stripe passes of the stacked open reproduce the fold-based
/// evaluation and the serial λ-combination exactly.
#[test]
fn stacked_open_stripe_passes_match_reference() {
    use crate::whir::stacked::{eq_weighted_sum, lambda_combination};
    use crate::whir::sumcheck::eq_table;
    use p3_field::BasedVectorSpace;

    type F = InnerVal;
    type EF = InnerChallenge;
    let n = 15usize;
    let mut rng = StdRng::seed_from_u64(0xE0A1);
    let stripes: Vec<Vec<F>> =
        (0..5).map(|_| (0..1usize << n).map(|_| rand_kb(&mut rng)).collect()).collect();
    let point: Vec<EF> = (0..n)
        .map(|_| {
            <EF as BasedVectorSpace<F>>::from_basis_coefficients_iter(
                (0..4).map(|_| rand_kb(&mut rng)),
        ...
        .collect();
    let eq = eq_table(n, &point);
    for s in stripes.iter() {
        let mle = Mle::from_row_major(RowMajorMatrix::new(s.clone(), 1));
        assert_eq!(eq_weighted_sum(&eq, s), mle.eval_at::<EF>(&point)[0]);
    }
    let lambda: EF = <EF as BasedVectorSpace<F>>::from_basis_coefficients_iter(
        (0..4).map(|_| rand_kb(&mut rng)),
    )
    .unwrap();
    let mut lam = EF::ONE;
    let mut weighted: Vec<(&[F], EF)> = Vec::new();
    let mut reference = vec![EF::ZERO; 1usize << n];
    for s in stripes.iter() {
        weighted.push((s.as_slice(), lam));
        for (v, &st) in reference.iter_mut().zip(s.iter()) {
            *v += lam * st;
        }
        lam *= lambda;
    }
    assert_eq!(lambda_combination(&weighted, 1usize << n), reference);
}
