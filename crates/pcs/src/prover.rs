    ...
    >: Send + Sync + 'static,
{
    ...
    fn commit(
        ...
        // single-commit path.  Only `main_commit` survives into the shard proof;
        // nothing ever opens against `main_data` (the shard proof carries no FRI
        // opening), and the BaseFold verifier reads its opening evidence from
