    ...
    A: Backend,
{
        ...
        E: Field,
    {
        ...
        let len = original_len * <T as BasedVectorSpace<E>>::DIMENSION;
        let cap = original_cap * <T as BasedVectorSpace<E>>::DIMENSION;
        ...
        E: ExtensionField<T>,
    {
        ...
        let (original_ptr, original_len, original_cap, allocator) =
            (buffer.as_mut_ptr(), buffer.len(), buffer.capacity(), buffer.allocator().clone());
        let ptr = original_ptr as *mut E;
        let len = original_len.checked_div(<E as BasedVectorSpace<T>>::DIMENSION).unwrap();
        let cap = original_cap.checked_div(<E as BasedVectorSpace<T>>::DIMENSION).unwrap();
        unsafe { Buffer::from_raw_parts(ptr, len, cap, allocator) }
    }
}
...
/// ```
///
/// [`Buffer`]: crate::tensor::Buffer
#[allow(unused_macros)]
macro_rules! buffer {
    ($($x:expr),*) => {
       $crate::tensor::Buffer::from(vec![$($x),*])
    };
}
