...
impl<F: PrimeField32, const DEGREE: usize> MachineAir<F> for SumcheckVerifyChip<DEGREE> {
    ...
    fn generate_trace(
        ...
        // One row per sumcheck round event.  Materialise the
        // SumcheckVerifyCols layout from each event's fields.
        use core::iter::repeat;
        let nb_rows = input.sumcheck_verify_events.len();
        let mut values: Vec<F> =
            Vec::with_capacity(nb_rows * NUM_SUMCHECK_VERIFY_COLS);

        for event in &input.sumcheck_verify_events {
            // Layout matches SumcheckVerifyCols: challenge, claimed_sum,
            // c0, c1, c2, new_claim.  Each is a Block<F> (D field elements).
            values.extend(event.challenge.0.iter().copied());
            values.extend(event.claimed_sum.0.iter().copied());
            values.extend(event.coeffs[0].0.iter().copied());
            values.extend(event.coeffs[1].0.iter().copied());
            values.extend(event.coeffs[2].0.iter().copied());
            values.extend(event.new_claim.0.iter().copied());
        }

        // Pad to power-of-two height with zero rows (preserves AIR
        // shape).
        let target_rows = if self.pad {
            nb_rows.next_power_of_two().max(1)
        } else {
            nb_rows
        };
        let pad_rows = target_rows.saturating_sub(nb_rows);
        values.extend(repeat(F::ZERO).take(pad_rows * NUM_SUMCHECK_VERIFY_COLS));

        Ok(RowMajorMatrix::new(values, NUM_SUMCHECK_VERIFY_COLS))
    }

    fn included(&self, shard: &Self::Record) -> bool {
        !shard.sumcheck_verify_events.is_empty()
    }
}
...
mod tests {
    ...
    use crate::SumcheckVerifyEvent;
    use crate::runtime::RecursionProgram;

    /// Validates that `SumcheckVerifyChip::generate_trace` materialises
    /// the expected number of rows from supplied events, padding to
    /// the next power-of-two height when `pad=true`.
    #[test]
    fn test_sumcheck_verify_chip_generate_trace_padding() {
        type F = KoalaBear;
        let chip: SumcheckVerifyChip<3> = SumcheckVerifyChip::default();

        // Build 3 events; pad should round up to 4 rows.
        let events: Vec<SumcheckVerifyEvent<F>> = (0..3)
            .map(|i| SumcheckVerifyEvent {
                claimed_sum: Block([F::from_u32(i * 10); 4]),
                coeffs: [
                    Block([F::from_u32(i * 10 + 1); 4]),
                    Block([F::from_u32(i * 10 + 2); 4]),
                    Block([F::from_u32(i * 10 + 3); 4]),
                ],
                challenge: Block([F::from_u32(i * 10 + 4); 4]),
                new_claim: Block([F::from_u32(i * 10 + 5); 4]),
            })
            .collect();

        let mut record: ExecutionRecord<F> = ExecutionRecord {
            program: Arc::new(RecursionProgram::default()),
            ..Default::default()
        };
        record.sumcheck_verify_events = events.clone();

        let mut output = ExecutionRecord::default();
        let trace = chip.generate_trace(&record, &mut output).expect("ok");

        assert_eq!(trace.width, NUM_SUMCHECK_VERIFY_COLS);
        assert_eq!(trace.values.len(), 4 * NUM_SUMCHECK_VERIFY_COLS); // padded
    }

    /// Validates that the `included` predicate returns `true` iff the
    /// shard has any sumcheck events.
    #[test]
    fn test_sumcheck_verify_chip_included() {
        type F = KoalaBear;
        let chip: SumcheckVerifyChip<3> = SumcheckVerifyChip::default();
        let mut record: ExecutionRecord<F> = ExecutionRecord::default();
        assert!(!chip.included(&record), "no events -> not included");
        record.sumcheck_verify_events.push(SumcheckVerifyEvent {
            claimed_sum: Block([F::ZERO; 4]),
            coeffs: [Block([F::ZERO; 4]); 3],
            challenge: Block([F::ZERO; 4]),
            new_claim: Block([F::ZERO; 4]),
        });
        assert!(chip.included(&record), "events present -> included");
    }

    /// Validates that empty-event input still produces a valid (zero-
    /// row or padded-zero-row) trace without panicking.
    #[test]
    fn test_sumcheck_verify_chip_empty_events() {
        type F = KoalaBear;
        let chip: SumcheckVerifyChip<3> = SumcheckVerifyChip::default();
        let record: ExecutionRecord<F> = ExecutionRecord::default();
        let mut output = ExecutionRecord::default();
        let trace = chip.generate_trace(&record, &mut output).expect("ok");
        assert_eq!(trace.width, NUM_SUMCHECK_VERIFY_COLS);
        // pad=true rounds 0 events up to 1 row of zero padding.
        assert_eq!(trace.values.len(), NUM_SUMCHECK_VERIFY_COLS);
    }
}
