...
namespace zkm_recursion_core_sys::poseidon2 {
...
template <class F>
__ZKM_HOSTDEV__ __ZKM_INLINE__ void mdsLightPermutation4x4(F state[4]) {
...
template <class F>
__ZKM_HOSTDEV__ __ZKM_INLINE__ void external_linear_layer(
...
__ZKM_HOSTDEV__ __ZKM_INLINE__ void internal_linear_layer(
  ...
  for (size_t i = 0; i < POSEIDON2_WIDTH; i++) {
    matmul_constants[i] = F(F::to_monty(F::from_monty(
        constants::POSEIDON2_INTERNAL_MATRIX_DIAG_16_KOALABEAR_MONTY[i].val)));
  }
