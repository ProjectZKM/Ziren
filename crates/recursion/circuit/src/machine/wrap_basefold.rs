//! SP1-style parallel call site for the wrap recursion stage.
//!
...
//! # Status
//!
//! Skeleton only — wrap is the final aggregation stage that
//! produces the BN254-compatible proof.  Full body wiring lifts
//! the wrap-specific transcript handling on top of the
//! `verify_shard` call established in
//! [`super::compress_basefold::verify_compress_basefold`].
...
pub struct ZKMWrapBasefoldWitnessValues<SC: zkm_stark::StarkGenericConfig> {
    ...
    pub proof: BasefoldShardProof<InnerVal, InnerChallenge>,
}
...
/// Witness variable type — single (vk, proof-tuple).
...
/// Verify a single wrap proof using BasefoldShardVerifier.
///
/// Wrap is the final aggregation stage that produces a
/// BN254-compatible proof — verifies a single input proof and
/// commits the public values.  No per-input loop unlike
/// compress/deferred.
        ...
        + for<'b> p3_air::Air<crate::basefold_constraint_folder::BasefoldConstraintFolder<'b, C>>,
{
    ...
    let ZKMWrapBasefoldWitnessVariable { vk: vk_legacy, proof_tuple } = input;
    let (
        main_commit,
        public_values,
        logup_gkr_proof,
