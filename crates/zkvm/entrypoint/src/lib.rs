...
///
/// If the input stream is exhausted, the failed flag is returned as true. In this case, the other outputs from the function are likely incorrect, which is fine as `zkm-lib` always panics in the case that the input stream is exhausted.
///
/// The `unsafe` accesses are sound because the VM is single threaded and the
/// layout comes from the checked constructor.
#[no_mangle]
