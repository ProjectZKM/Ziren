//! The x86_64-Linux native minimal-trace producer.
//!
//! Split out of `jit_producer.rs` for the same reason as
//! [`crate::jit_runner::platform`]: one `#[cfg]` over the bulk of the file.
//! The parent keeps the `cfg` on the `mod` declaration and the interpreter
