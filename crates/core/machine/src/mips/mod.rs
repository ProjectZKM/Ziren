...
pub mod tests {
    ...
    fn test_subu_prove() {
        utils::setup_logger();
        let instructions = vec![
            Instruction::new(Opcode::ADDU, 29, 0, 5, false, true),
            Instruction::new(Opcode::ADDU, 30, 0, 8, false, true),
            Instruction::new(Opcode::SUBU, 31, 30, 29, false, false),
        ];
        let program = Program::new(instructions, 0, 0);
        run_test::<CpuProver<_, _>>(program).unwrap();
    }
    ...
    fn test_add_overflow_prove() {
        setup_logger();
        let instructions = vec![
            Instruction::new(Opcode::ADD, 29, 0, 3_000_000_000, false, true),
            Instruction::new(Opcode::ADD, 30, 0, 3_000_000_000, false, true),
            Instruction::new(Opcode::ADD, 31, 30, 29, false, false),
        ];
        let program = Program::new(instructions, 0, 0);
        run_test::<CpuProver<_, _>>(program).unwrap();
