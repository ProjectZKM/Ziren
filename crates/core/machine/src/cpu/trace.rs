...
// NOTE(frame refactor): the FFI parity test for `CpuChip` was removed —
// the chip emits ZERO rows now (every instruction chip owns its frame), and
// the whole cpu module goes away when `Cpu` is dropped from `MipsAir`.
