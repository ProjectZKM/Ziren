...
impl<F: PrimeField32> CoreShapeConfig<F> {
    ...
    /// raw-height normalize VK equals the fixed-shape canonical cluster VK in the
    /// production vk_map.
    ///
    ...
    /// (`utils/prove.rs`, the raw-height jagged commit) uses only the chip NAMES,
    /// and a `None` there leaves the shard committed at its RAW present-chip
    /// set, whose normalize VK the enumeration never emits.  Example: the fib
    /// ELF proved by `build_groth16_bn254`'s `dummy_proof()` has one packed
    /// shard that fits no `partial_small_shapes` band; without the fallback it
    /// commits 23 chips instead of the 27-chip `memory` cluster and shrink
    /// rejects it under `VERIFY_VK=true`.
    ...
    ///
    /// Heights here are deliberately meaningless — the raw-height commit reads
    ...
    /// guard's missing-chip injection), so its raw-height normalize VK equals the
    ...
    /// band-cap installed ⇒ each chip is packed at its own height).
    ...
    /// worker); separates precompile-shard worker chips from the shared
    ...
    /// the exact heights the real raw-height commit used.
    ...
    /// a raw-height core proof can lift to — config-driven, proof-independent.
    ///
    ...
    /// raw-height normalize VK.  We drive `find_canonical_cluster_shape_from_raw`
    ...
    /// Per-chip cartesian enumeration of all shard shapes (~1.25M for MIPS).
    ///
    /// VK generation (`ZKMProofShape::generate`) does not use it; it uses
    /// `zkm_pcs::stacked_shapes::create_all_input_shapes` (≤ 5,000 shapes,
    /// size-class quantization).  This view serves the `#[ignore]`d
    /// `test_making_shapes` and diagnostics.
...
/// log-height 1.  Shared by the record post-pass (fixed-shape) and the read-only
/// [`CoreShapeConfig::find_canonical_cluster_shape`] (raw-height band-cap).
...
pub mod tests {
    ...
    /// the CPU-shard canonical shape a real raw-height fib-1k core proof lifts to
    ...
    /// family cluster (inject ShaCompress + ShaCompressControl), so its raw-height
