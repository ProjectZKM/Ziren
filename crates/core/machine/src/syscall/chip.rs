    ...
    fn generate_dependencies(&self, input: &ExecutionRecord, output: &mut ExecutionRecord) {
        let events = match self.shard_kind {
            SyscallShardKind::Core => &input
                .syscall_events
                .iter()
                .filter(|e| (e.a_record.prev_value.to_le_bytes()[2] == 1) || (e.a_record.prev_value.to_le_bytes()[1] != 0))
                .copied()
                .collect::<Vec<_>>(),
    ...
    fn generate_trace(
        ...
        let mut rows = match self.shard_kind {
            SyscallShardKind::Core => input
                .syscall_events
                .par_iter()
                .filter(|event| (event.a_record.prev_value.to_le_bytes()[2] == 1) || (event.a_record.prev_value.to_le_bytes()[1] != 0))
