...
// Route every exit through zkvm·RuntimeExit, which sums PublicValuesHasher,
// emits the eight COMMIT syscalls that publish sha256(public values), and then
// halts via SyscallExit (v0 = 0).
//
// It used to be reached implicitly, anchored only by `_ = reflect.ValueOf(
// RuntimeExit)` in zkvm_runtime's init().  Go >= 1.25's linker prunes that:
// `go tool nm` shows zkvm.RuntimeExit, SyscallCommit and SyscallExit all gone
// from the guest, so the program ends here at exit_group having committed
// nothing.  The host still has the public-values stream (Commit writes fd 13),
// but committed_value_digest is never published, and verification fails with
// InvalidPublicValues (sdk/provers/mod.rs:166).  Anchoring the func in Go --
// an exported package-level var holding it -- does NOT survive either; the
// reference has to come from code the linker cannot drop.
//
// This JAL is that reference: it both pins the symbol and guarantees the
// commit-then-halt runs on every exit path.  On the normal path runtime.main
// calls exit(0) from the main goroutine, so RuntimeExit runs on a growable,
// allocating stack as it always did.
TEXT runtime·exit(SB),NOSPLIT,$4-4
	MOVW	code+0(FP), R1
	MOVW	R1, 4(R29)
	MOVW	$zkvm·RuntimeExit(SB), R2
	JAL	(R2)
	// RuntimeExit halts and does not return.  Should it ever return, fall
	// back to the raw kernel exit rather than spinning.
	MOVW	code+0(FP), R4
