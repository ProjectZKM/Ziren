...
fn main() {
    ...
    let elf =
        fs::read(&elf_path).unwrap_or_else(|e| panic!("failed to read {:?}: {}", elf_path, e));
    let stdin_bytes =
        fs::read(&stdin_path).unwrap_or_else(|e| panic!("failed to read {:?}: {}", stdin_path, e));
    let stdin: ZKMStdin = bincode::deserialize(&stdin_bytes).expect("failed to deserialize stdin");
    ...
    let total_bytes: usize =
        proof.proof.0.iter().map(|shard| bincode::serialize(shard).unwrap().len()).sum();
