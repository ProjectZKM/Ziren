...
impl PlonkVerifier {
    /// Verifies an zkMIPS PLONK proof, as generated by the zkMIPS SDK.
    ///
    /// # Arguments
    ///
    /// * `proof` - The proof bytes.
    /// * `public_inputs` - The zkMIPS public inputs.
    /// * `zkm_vkey_hash` - The zkMIPS vkey hash.
    ...
    pub fn verify(
        ...
        // the given plonk vk.
        //
        // zkMIPS prepends the raw Plonk proof with the first 4 bytes of the plonk vkey to
    ...
    ///
    /// WARNING: if you're verifying an zkMIPS proof, you should use [`verify`] instead.
    /// This is a lower-level verification method that works directly with raw bytes rather than
    /// the zkMIPS SDK's data structures.
    ///
    /// # Arguments
    ///
