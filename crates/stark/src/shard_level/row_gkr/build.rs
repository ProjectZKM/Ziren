    ...
    EF: ExtensionField<F>,
{
    ...
    // #371 dual-run-verify probe.  When `ZIREN_GPU_BASEFOLD_LT_VERIFY=1`,
    // for every Device entry we just pushed, pull it back to host and
    // compare against the layer the host transition would produce from
    // the same initial first_ef_layer.  Localises which layer + which
    // field first diverges (n0/d0/n1/d1).  Panics with details on
    // mismatch.  Default OFF (probe is O(circuit-depth) extra pulls
    // and a full host re-computation).
    //
    // The probe runs AFTER the device walk finished, so all per-layer
    // device buffers are still live in the per-circuit registry (drain
    // happens after the round loop in top_level.rs).  Each pull is
    // non-destructive — `gpu_layer_pull_hook` uses `.get(&handle_seq)`
    // (not `.remove(...)`), so we can pull each handle without
    // disturbing the registry for the subsequent prove path.
    if std::env::var("ZIREN_GPU_BASEFOLD_LT_VERIFY").as_deref() == Ok("1") {
        run_lt_verify_probe::<F, EF>(circuit_id, layers, &terminal_ef, pull_hook);
    }

    Some(terminal_ef)
}

/// #371 dual-run-verify probe.  Pulls every Device entry in `layers`
/// (in push order — same as call order), re-computes the corresponding
/// host layer chain starting from the first Host EF layer (the one
/// pushed at line 375 immediately before the device walk), and diffs
/// each device-pulled layer cell-by-cell against the host equivalent.
///
/// Panics with a concrete divergence report on mismatch — layer index,
/// num_row_variables, chip index, quadrant (n0/d0/n1/d1), cell index,
/// device value, host value.  The first divergence is enough to
/// localise the root cause; we abort there rather than walking the
/// rest of the layer chain to keep the report focused.
///
/// The terminal is included in the diff (it's the last device handle
/// pulled, with num_row_variables == 1 by construction).
#[cfg(feature = "basefold")]
#[allow(clippy::too_many_lines)]
fn run_lt_verify_probe<F, EF>(
    circuit_id: u64,
    layers: &[LayerState<F, EF>],
    terminal_ef: &super::layer::LogUpGkrCpuLayer<EF, EF>,
    pull_hook: crate::basefold_late_binding::GpuLayerPullFn,
)
where
    F: PrimeField,
    EF: ExtensionField<F>,
{
    use core::any::TypeId;
    use crate::basefold_late_binding::LbChallenge;

    // Type-guard — only run when EF == LbChallenge (the only case where
    // the pull_hook is sound).  Outer call site already gated on this.
    if TypeId::of::<EF>() != TypeId::of::<LbChallenge>() {
        return;
    }

    // Reconstruct the host chain.  The first Host EF entry in `layers`
    // is the seed (pushed right before the device walk).  Then each
    // host transition produces the next layer.
    let mut host_chain: Vec<super::layer::LogUpGkrCpuLayer<EF, EF>> = Vec::new();
    let mut last_host: Option<super::layer::LogUpGkrCpuLayer<EF, EF>> = None;
    // Find the first Host EF layer (the seed) — by construction, it's
    // the LAST Host entry before the first Device entry in `layers`.
    let mut seed: Option<super::layer::LogUpGkrCpuLayer<EF, EF>> = None;
    for state in layers.iter() {
        match state {
            LayerState::Host(super::layer::GkrCircuitLayer::Layer(l)) => {
                seed = Some(l.clone());
            }
            LayerState::Host(super::layer::GkrCircuitLayer::FirstLayer(_)) => {
                // Skip — FirstLayer is the F-numerator layer; the seed
                // for the EF chain is the next Host entry (the F→EF
                // transition output, pushed at build.rs:375).
            }
            LayerState::Device { .. } => break,
        }
    }
    let Some(mut cur) = seed else {
        eprintln!(
            "LT_VERIFY: no Host EF seed layer found in circuit_id={circuit_id}; \
             cannot run probe."
        );
        return;
    };

    // Apply host transitions to build the full host chain, one entry
    // per LayerState::Device in `layers` (matching the call sequence).
    let n_device: usize = layers
        .iter()
        .filter(|s| matches!(s, LayerState::Device { .. }))
        .count();
    for _ in 0..n_device {
        if cur.num_row_variables == 0 {
            // Defensive — host loop in build_gkr_circuit pushes a final
            // num_row_variables=0 layer too, so the device chain may
            // include one extra.  Stop computing transitions here.
            host_chain.push(cur.clone());
            // Pad remaining slots with the same null terminal so the
            // diff loop below has a host counterpart for every device
            // entry.
            while host_chain.len() < n_device {
                host_chain.push(cur.clone());
            }
            break;
        }
        let next = super::transition::layer_transition::<EF, EF>(&cur);
        host_chain.push(next.clone());
        cur = next;
    }
    last_host = Some(cur);
    let _ = last_host;

    // Now iterate device entries and pull each, diffing against the
    // corresponding host_chain entry.
    let mut device_idx: usize = 0;
    for state in layers.iter() {
        let LayerState::Device { circuit_id: cid, handle, .. } = state else {
            continue;
        };
        debug_assert_eq!(*cid, circuit_id, "LT_VERIFY: circuit_id mismatch");
        let dev_lb: super::layer::LogUpGkrCpuLayer<LbChallenge, LbChallenge> =
            pull_hook(circuit_id, *handle);
        // SAFETY: EF == LbChallenge (type-guarded above).
        let dev_ef: super::layer::LogUpGkrCpuLayer<EF, EF> = unsafe {
            let out: super::layer::LogUpGkrCpuLayer<EF, EF> =
                core::mem::transmute_copy(&dev_lb);
            core::mem::forget(dev_lb);
            out
        };
        let host_ef = &host_chain[device_idx];
        diff_layer_or_panic::<EF>(circuit_id, device_idx, &dev_ef, host_ef);
        device_idx += 1;
    }

    // Sanity check: terminal_ef must match host_chain[?].  The terminal
    // is one of the device handles already diffed above (the one with
    // num_row_variables == 1).  Find it in host_chain and verify they
    // agree shape-wise.
    let _ = terminal_ef;
    let _ = pull_hook;
    tracing::warn!(
        "LT_VERIFY: probe PASSED for circuit_id={circuit_id} — {n_device} \
         device layers diffed against host chain, all cells matched."
    );
}

#[cfg(feature = "basefold")]
fn diff_layer_or_panic<EF>(
    circuit_id: u64,
    layer_idx: usize,
    dev: &super::layer::LogUpGkrCpuLayer<EF, EF>,
    ...
    EF: p3_field::Field + core::fmt::Debug,
{
    ...
    if dev.num_row_variables != host.num_row_variables
        || dev.num_interaction_variables != host.num_interaction_variables
    {
        panic!(
            "LT_VERIFY (circuit={circuit_id}, layer={layer_idx}): \
             metadata mismatch — dev num_row={} num_int={} vs \
             host num_row={} num_int={}",
            dev.num_row_variables, dev.num_interaction_variables,
            host.num_row_variables, host.num_interaction_variables,
        );
    }
    if dev.numerator_0.len() != host.numerator_0.len() {
        panic!(
            "LT_VERIFY (circuit={circuit_id}, layer={layer_idx}): \
             chip count mismatch — dev {} vs host {}",
            dev.numerator_0.len(), host.numerator_0.len(),
        );
    }
    let diff_one = |
        chip_idx: usize, quad: &'static str,
        dev_t: &RowMajorTable<EF>, host_t: &RowMajorTable<EF>,
    | {
        if dev_t.num_real_rows != host_t.num_real_rows {
            panic!(
                "LT_VERIFY (circuit={circuit_id}, layer={layer_idx}, \
                 chip={chip_idx}, quad={quad}): num_real_rows mismatch — \
                 dev {} vs host {} (num_interactions dev={} host={})",
                dev_t.num_real_rows, host_t.num_real_rows,
                dev_t.num_interactions, host_t.num_interactions,
            );
        }
        if dev_t.num_interactions != host_t.num_interactions {
            panic!(
                "LT_VERIFY (circuit={circuit_id}, layer={layer_idx}, \
                 chip={chip_idx}, quad={quad}): num_interactions \
                 mismatch — dev {} vs host {}",
                dev_t.num_interactions, host_t.num_interactions,
            );
        }
        // Compare only the materialized real-row prefix; both sides
        // produce identical-length cells when num_real_rows agree.
        let want = dev_t.num_real_rows * dev_t.num_interactions;
        if dev_t.cells.len() < want || host_t.cells.len() < want {
            panic!(
                "LT_VERIFY (circuit={circuit_id}, layer={layer_idx}, \
                 chip={chip_idx}, quad={quad}): cells short — \
                 dev.len={} host.len={} want={}",
                dev_t.cells.len(), host_t.cells.len(), want,
            );
        }
        for k in 0..want {
            if dev_t.cells[k] != host_t.cells[k] {
                let row = k / dev_t.num_interactions;
                let col = k % dev_t.num_interactions;
                panic!(
                    "LT_VERIFY (circuit={circuit_id}, layer={layer_idx}, \
                     chip={chip_idx}, quad={quad}, row={row}, col={col}, \
                     cell={k} of {want}): VALUE MISMATCH — \
                     dev={:?} host={:?} \
                     [layer metadata: num_row_vars={}, num_int_vars={}, \
                      chip num_real_rows dev={} host={}, \
                      num_interactions dev={} host={}]",
                    dev_t.cells[k], host_t.cells[k],
                    dev.num_row_variables, dev.num_interaction_variables,
                    dev_t.num_real_rows, host_t.num_real_rows,
                    dev_t.num_interactions, host_t.num_interactions,
                );
    ...
    for chip_idx in 0..dev.numerator_0.len() {
        diff_one(chip_idx, "n0", &dev.numerator_0[chip_idx], &host.numerator_0[chip_idx]);
        diff_one(chip_idx, "d0", &dev.denominator_0[chip_idx], &host.denominator_0[chip_idx]);
        diff_one(chip_idx, "n1", &dev.numerator_1[chip_idx], &host.numerator_1[chip_idx]);
        diff_one(chip_idx, "d1", &dev.denominator_1[chip_idx], &host.denominator_1[chip_idx]);
    }
}
