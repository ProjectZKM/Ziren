...
fn render_constraint(c: &PicusConstraint) -> String {
    match c {
        ...
        PicusConstraint::Lt(a, b) => match b.as_ref() {
            PicusExpr::Const(c) => format!("({}).val < {}", render_expr(a), c % KB),
            _ => format!("({}).val < ({}).val", render_expr(a), render_expr(b)),
        },
...
fn write_module(
    ...
    // heartbeat timeout used to be uncatchable (now `picus_safe` admits the goal instead) and the
    // attempt still costs the whole budget.  Above the threshold the obligation is stated and left open explicitly.
    ...
    // The selector-shape module `top` has no inputs: its selectors are not a function of
    // anything, so only its postconditions are meaningful.
    if m.name != "top" {
...
pub fn write_chip(
    ...
    writeln!(
        f,
        "/-\n  Generated by `cargo run -p zkm-fv -- --chip {chip} --format lean`.\n  \
