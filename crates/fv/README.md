...
`zkm-fv` turns every chip of the MIPS machine into a set of *determinism modules* and writes
them as
...
```bash
cargo run -p zkm-fv -- --list
cargo run -p zkm-fv -- --chip AddSub --picus-out-dir picus_out --lean-out-dir crates/fv/lean
cargo run -p zkm-fv -- --all --format lean --lean-out-dir crates/fv/lean
```
...
`crates/fv/lean` is a Lake project pinned to Mathlib `v4.33.1`.  For every module `M` the
