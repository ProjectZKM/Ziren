...
The cost model of \S\ref{sec:perf-profile} suggested the changes in Table~\ref{tab:levers}. Replacing the 30-bit boolean sign decomposition, eight offset bits and seven chord-witness columns of the \texttt{Global} chip with three byte lookups removed 42 columns from 48\,M rows and 3.9\% of wall-clock time. The instrumented compressed-proof measurement remained 617\,618 bytes, while the core proofs, some $10^{8}$ bytes per block, shrank by 3.4\%. Removing redundant byte checks on memory-instruction accesses removed 5.7\% of the \LogUp{} pairs and 1.4\% of wall-clock time. This is less than the two-cells-per-pair model predicts because the lookup circuit's fold layers scale with padded heights rather than pair count. Reducing a 20\,M-row chip by 11 columns was below measurement noise; the 1.0\,s byte-check result was at its edge.

The largest single effect came from shard size. Raising it by a quarter removed 16 shards and saved 3.3\,s, approximately 0.2\,s per shard. Some shards close early on a chip's height budget, so the reduction is smaller than the cycle fence alone suggests. GPU memory limited larger shards at this revision: at 1.5 times the baseline size, the 4.5\,GiB first-layer \LogUp{} buffer caused repeated trim-and-retry and a 30\% slowdown; twice the baseline size did not fit in 32\,GB.
