...
Figure~\ref{fig:processor} shows instruction units connected by buses to a program ROM, memory subsystem and lookup tables. For a unit in the sense of Definition~\ref{def:chip}, the column count is its datapath width and the row count is its activity for an execution. A \emph{bus} is an interconnect on which units exchange tuples. The lookup argument of Section~\ref{sec:air-logup} acts as the wiring by proving that every tuple driven on a bus is consumed exactly once. The area of the design is the sum over units of rows $\times$ columns (\emph{trace cells}); Section~\ref{sec:performance} reports it per unit, as a gate-count report would.

\begin{figure}[t]
\centering
\resizebox{\textwidth}{!}{\input{figures/processor}}
\caption{Each executed instruction occupies one opcode-unit row, and every relation between units crosses a named bus. The program and lookup tables are preprocessed; public values close the state chain and carry the cross-shard digests.}
\label{fig:processor}
\end{figure}

This section defines the constrained execution relation within the system boundary of Section~\ref{sec:ipcore}. The unit widths determine its per-unit trace area (Appendix~\ref{sec:app-units}), and the buses connect the units. One row per instruction, one shared memory argument, one lookup and one zerocheck fix the area measured in Section~\ref{sec:performance}. The guest-accelerator set is configuration rather than fixed design: it determines which precompile units a shard may contain, and adding one changes the unit set and verifying keys (\S\ref{sec:ipcore-config}).

A shard is proved as a set of units. A unit is a fixed-width table over $\F$ with polynomial constraints of degree at most three on each row and interactions on named buses. Constraints are local to one row and evaluated at a random point of the multilinear domain (Section~\ref{sec:air-zerocheck}), so relations between rows are expressed as bus interactions. Every relation between units, or between different rows of one unit, is an interaction. This section describes the unit set, instruction frame, register and memory argument, lookup argument and zerocheck.

\subsection{Unit set}
\label{sec:air-chips}

Appendix~\ref{sec:app-units} catalogues the units of the machine measured here, with their widths. Three families dominate the trace area of an Ethereum block: the memory-instruction units (\texttt{LoadWord}, \texttt{StoreWord}, \texttt{MemoryUnaligned}, \texttt{LoadNarrow}, \texttt{StoreNarrow}), the ALU units, and the \texttt{Global} unit that carries the cross-shard memory argument. The immediate forms of the ALU instructions (\texttt{ADDIU}, \texttt{ANDI}, \texttt{SLTI}, shifts by a constant, \ldots) have units separate from the register forms: an immediate row has one register read fewer and a narrower frame, and the split saves roughly a tenth of the width of those rows.
...
A design organised around a central \emph{CPU table}, as this system's first generation was, has one row per executed cycle that fetches the instruction, performs the register accesses, chains the program counter and forwards a decoded instruction to an opcode unit over a bus; the opcode unit then spends a second row on the same instruction. The current design removes the central table: each instruction unit embeds an \emph{instruction frame} and performs those duties itself, so one executed instruction is exactly one row of one unit. The frame consists of:
...
\centering
\resizebox{\textwidth}{!}{\input{figures/frame_flow}}
...
\centering
\resizebox{\textwidth}{!}{\input{figures/frame_unit}}
...
\begin{figure}[!tbp]
\centering
\resizebox{\textwidth}{!}{\input{figures/memory_chain}}
...
The lift is a map on \emph{pairs}, not on messages: $\mathsf{enc}(\bm m, \mathsf{offset})$. The offset is a witness column, and the unit constrains only that it is a byte occupying the low positions of $x_6$; it does not constrain it to be canonical. The honest prover takes the smallest admissible offset. The implementation scans $\mathsf{offset} = 0, \dots, 255$ and stops at the first value whose $x$ makes $x^3 + 3\zeta x - 3$ a square in $\F_{p^7}$, so that the point exists. Minimality is not an AIR constraint, so a satisfying trace may carry any admissible offset. We therefore write $\mathcal P$ for the \emph{reachable pairs} $(\bm m, \mathsf{offset})$ that an interaction of a satisfying trace can carry, given the range facts each sending unit enforces and the byte bound on the offset, and reserve $\mathcal M$ for the projection of $\mathcal P$ onto messages. $\mathcal M$ is far smaller than $\F^7$, since every limb of a memory or syscall tuple is a byte, a bounded clock limb or a bounded address, and the whole argument rests on that; the offset multiplies the reachable set by at most $256$.
