...
program are table dispatches, so the compiled chip is the whole program and a
...
\begin{itemize}
  ...
  and the per-chip height budgets in the context are charged the exact amounts
...
\midrule
Per-cycle event: keep clock, \textsf{pc} pair and exit code only (no CPU chip reads the rest) & 184\,B & 20\,B & event stream per shard 1{,}000\,MB $\to$ 462\,MB; replay 700\,ms $\to$ 435\,ms \\
