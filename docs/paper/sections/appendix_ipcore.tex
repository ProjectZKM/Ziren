...
The proving system is parameterised, and Section~\ref{sec:performance} walks through part of its configuration space. Parameters include shard size, commitment stacking height, the \WHIR{} schedule, lookup-challenge grinding, recursion arity and guest-accelerator set. The \WHIR{} schedule fixes committed rounds, folding factors, code rates, queries, out-of-domain samples and query grinding, thereby setting proof size and soundness accounting (Section~\ref{sec:iopp}). The guest-accelerator set determines precompile units and their verifying keys. Release values belong in one configuration file. A constraint-system parameter changes the affected unit keys and the recursion-key allowlist; a schedule-only parameter changes recursion-program keys but not unit keys.
