...
Theorem~\ref{thm:unique} is what makes a satisfying trace unique given the determinism of every chip. The theorems below are its premises, machine-checked for the chips the automation closes and stated with an explicit gap for the rest.

\paragraph{Extraction.} The theorems are not written by hand. The same Rust constraint description that the prover evaluates and the verifier checks is evaluated once with symbolic columns, the resulting polynomial constraints, range facts and bus interactions are lowered to a first-order statement, and a generator emits one \Lean{} file per chip: a record type for the row, the constraints as a conjunction, the input and output projections with their provenance, and the theorem above. Selector-specialised chips (one theorem per opcode selector) and an \emph{is\_real}~$=1$ specialisation keep each statement about real rows of one opcode. The extraction covers every chip of the core machine catalogued in Appendix~\ref{sec:app-chips}: \ndettotal{} determinism theorems.
...
\paragraph{Coverage.} At the time of writing \ndetproved{} of the \ndettotal{} theorems are closed (Figure~\ref{fig:verification}, by chip family). \ndetopen{} were attempted and remain open: integer chips whose constraints multiply by $2^{24}$ or $2^{32}$ and are only meaningful modulo $p$ (multiply, the conditional moves, the memory-argument chips), which the integer lifting does not model, or whose selector case structure exceeds the split budget. \ndetskipped{} are not attempted by the generator: \ndetskipacc{} are accelerator chips above the size threshold (the same theorem at 1\,000--5\,600 conjuncts), and \ndetskipcase{} are chips whose constraints contain comparison case-splits from the byte-table summaries, which the current tactic does not handle. \ndetpending{} had not finished their attempt when the numbers were taken. The syscall chip, which was under-determined as first extracted (next paragraph), closes after the bus change; the count above is therefore for the constraint system with that change applied. Two chips separate it from the system measured in Section~\ref{sec:performance}: that one, and the hash chip, whose round schedule was corrected afterwards (Table~\ref{tab:defects}). All theorem statements compile; open ones carry an explicit \texttt{sorry} and are listed in the artifact.

\begin{figure}[t]
\centering
\input{figures/verification}
\caption{The current artifact closes \ndetproved{} of \ndettotal{} extracted chip and opcode-selector determinism statements. The remaining statements are open, skipped because of accelerator size or comparison case splits, or unfinished under the fixed automation budget.}
...
of a chip's height, lies about which chips are active, tampering with the
...
\paragraph{Syscall argument half-words.} The syscall chip receives two reduced 32-bit arguments from the instruction that raised the syscall and witnesses each of them, written $a \in \F$ here, as two 16-bit half-words $a_{\mathrm{lo}}, a_{\mathrm{hi}}$ constrained by $a_{\mathrm{lo}} + 2^{16} a_{\mathrm{hi}} = a$ in $\F$ and two 16-bit range checks. Because $2^{32} > p$, this does not determine the half-words: for every $a \in \F$ both the decomposition of $a$ and that of $a + p$ satisfy the constraints, since $2p < 2^{32}$. The extracted theorem for the chip is false and the automation reports exactly these four half-word columns as open. Whether the ambiguity is exploitable depends on the consumer: an accelerator that re-reduces the half-words modulo $p$ sees the same field element either way, while one that uses them as an exact 32-bit pointer does not. The finding is a specification gap between the two sides of the syscall bus. The remedy, applied after the finding, is to carry the half-words rather than the reduced sum on the bus, together with the chip's Linux selector; with that change the chip's determinism theorem closes in under a minute. The change alters the constraints of two chips and therefore the verifying keys, so it is staged with the next key-changing release rather than deployed alone.
