...
\begin{itemize}
  ...
  per-chip height budgets, whichever of which binds first closing a shard
  (\S\ref{sec:isa}).
  \item The guest-accelerator set, which fixes the precompile chips a shard may
  contain and therefore the chip set itself (\S\ref{sec:ipcore-config}).
...
It is built from four primitives. A \emph{chip} is a table of columns over
$\F$ with a constraint system and a set of bus interactions
(Definitions~\ref{def:chip} and~\ref{def:bus}), and a shard is a set of chips
with a shared public-value row; chips are data rather than algorithms, so they
...
\begin{description}
  ...
  $v_{i,j}$ of column $j$ of chip $i$ at $\bm z$.
...
has the same shape and certifies instead that every chip's constraints vanish
...
to one evaluation of one polynomial, the number of chips, their widths and
their heights are invisible to the opening, and a chip may be added, widened or
...
program and public values agree up to permutation, given chip determinism and
...
Neither statement is about a particular chip set, a particular field or a
