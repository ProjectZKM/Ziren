...
This section measures the two efficiencies of \S\ref{sec:intro} in one deliberately narrow environment, and separates the front end from the back end within the second. \emph{Instruction efficiency} is the rows and trace cells an executed instruction occupies, which is a property of the arithmetisation and is reported as area (\S\ref{sec:perf-changes}). \emph{Proving efficiency} is the rate at which those cells become a proof, on named hardware (\S\ref{sec:perf-prover}); its front end, the rate at which the software emulator turns a guest program into the prover's input record together with the host memory and processor time that costs, is reported separately (\S\ref{sec:perf-emu}), because on a host driving several devices it is what binds. The environment uses one workload, one GPU model, one host and interleaved repetitions on the same input so that differences between configurations are attributable.
...
\paragraph{Metrics.} The two efficiencies of \S\ref{sec:intro} are measured by different quantities: \emph{trace area} and the rows behind it are instruction efficiency, while \emph{proving throughput} and the kernel profile are proving efficiency. \emph{Wall-clock time} is the end-to-end proving-process time from reading the cached input to writing the compressed proof. It includes host verification of every shard proof and the compressed proof, which accounts for 2--3\,s and would be omitted by a production service. \emph{Emulation rate} is guest cycles divided by the emulator's wall-clock time. \emph{Proving throughput} is guest cycles divided by the whole proving process's wall-clock time. The latter includes the former, so the rates are not directly comparable; neither is a clock frequency. \emph{Trace area} is the sum over chips of rows $\times$ columns, in cells, from the emulator's shard-close census and the chip widths. Table~\ref{tab:chips}, in Appendix~\ref{sec:app-chips}, gives the widths of the release this paper describes, while Table~\ref{tab:census} uses those of the evaluated revision, which precede the changes of \S\ref{sec:perf-changes}; the appendix says where the two differ. \emph{Proof size} is the byte length of the serialised compressed proof. A ten-shard \texttt{nsys} sample attributes GPU kernel time to kernel groups.
...
\subsection{Proving efficiency: where the cost is}
\label{sec:perf-prover}
\label{sec:perf-profile}

\begin{table}[t]
\centering
\caption{In the evaluated pre-correction baseline, \LogUp{} consumes 42\% of measured kernel time and the \texttt{Global} chip consumes 24.5\% of the 19.6\,G-cell trace area. Kernel shares come from a ten-shard \texttt{nsys} sample; area is rows $\times$ columns at the baseline shard count.}
\label{tab:census}
...
Zerocheck, interpreted (wide precompile chips) & 7.1\% & \texttt{StoreWord} (46\,M $\times$ 53) & 12.6\% \\
...
Table~\ref{tab:census} pairs the kernel-time profile with the area census; Figure~\ref{fig:profile} plots them side by side. Kernels account for approximately 340\,ms of the 500\,ms GPU time per shard, with launch gaps and host synchronisation making up the difference. The lookup argument is the largest kernel consumer, and its cost scales with \emph{(row, interaction)} pairs. This block has 7.8\,G pairs and 19.6\,G trace cells, suggesting that one pair costs approximately two cells. Memory-instruction chips carry 29 interactions per row, including 21 byte-table lookups, and account for 48\% of all pairs. In contrast, the \texttt{Global} chip holds one quarter of trace area but only 2.5\% of pairs because every touched word costs two rows. The resulting cost model uses area for commitments and zerocheck, and pairs for lookup, with one pair weighted as two cells.

\begin{figure}[t]
\centering
\begin{subfigure}[t]{0.49\textwidth}\centering\resizebox{\linewidth}{!}{\input{figures/area_bar}}\caption{Trace area by chip (19.6\,G cells); \texttt{Global} and the three largest memory-instruction chips hold 59\%.}\label{fig:area}\end{subfigure}\hfill
\begin{subfigure}[t]{0.49\textwidth}\centering\resizebox{\linewidth}{!}{\input{figures/kernel_bar}}\caption{\LogUp{} is the largest kernel group at 42\%, ahead of \WHIR{} commit and open at 19\% (nsys, ten-shard sample).}\label{fig:kernel}\end{subfigure}
\caption{The \texttt{Global} and three largest memory-instruction chips hold 59\% of trace area, while \LogUp{} is the largest measured kernel group at 42\%. Both panels describe the evaluated pre-correction baseline before the changes of \S\ref{sec:perf-changes}.}
\label{fig:profile}
\end{figure}
...
interactions from the chips the census names and one that raises the shard
...
witness columns in the cross-shard digest chip with three byte lookups, and
dropping byte checks on memory-instruction accesses that another chip already
