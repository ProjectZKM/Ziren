...
names the unit that arithmetises the instruction, so the table fixes which
rows of Appendix~\ref{sec:app-units} a program can occupy, and with it the
...
general registers are additions of an immediate zero rather than a unit of
their own. \texttt{SYNC}, \texttt{SYNCI} and \texttt{PREF} are accepted and
retire as no-ops, for which the same unit suffices. \texttt{SYSCALL} is absent
