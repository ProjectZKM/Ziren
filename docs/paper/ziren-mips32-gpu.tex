...
\begin{abstract}
\Ziren{} is a zero-knowledge virtual machine for MIPS32 (Release~2) execution and the second generation of zkMIPS~\citep{zkmips}, which was the first zkVM for the MIPS32 instruction set itself rather than for a MIPS-like model. It proves the execution of arbitrary MIPS32 programs, and the architecture rewards that choice: Release~2 is finished, so an arithmetisation pinned to it never chases a moving instruction set and the verifying keys derived from it stay valid; its fixed-width encodings in a handful of formats let one row of one unit carry an instruction's whole frame, with no central control table; and every compiler that already targets MIPS32 reaches the guest without a bespoke backend, so Rust, Go and C programs become the same ELF. Its delayed branches make a row's successor data rather than the following address, which shapes everything above.
...
Soundness is composed by a union bound over each profile's two dozen transcript components rather than quoted as the smallest of them: $100.70$ bits for a core shard proof, $100.76$ for a recursion node, $100.83$ for the outer wrap, $99.18$ across the three, and $93.50$ once every node of a measured block's tree is charged. All of these bound the \emph{interactive} protocol: Fiat--Shamir subtracts $\log_2 Q$ against an adversary making $Q$ oracle queries, and the cross-shard digest is an assumption rather than an error term. An executable Lean~4 ISA model and extracted determinism theorems close \ndetproved{} of \ndettotal{} cases and identify a genuine under-determination at the syscall bus. One NVIDIA RTX~5090 proves a 288-million-cycle Ethereum block at $5.9$\,MHz and four reach $20$\,MHz on 420 million cycles, with each engineering change measured against the configuration it replaced; those timings precede a correction to the $\Poseidon{}$ round schedule, and \S\ref{sec:ipcore} states which claim attaches to which revision.
\end{abstract}
...
\input{sections/035_ipcore}
\input{sections/037_argument}
\input{sections/04_air}
\input{sections/05_pcs}
\input{sections/06_iopp}
\input{sections/07_recursion}
\input{sections/075_verification}
\input{sections/08_performance}
...
most from: the decomposition into one unit per opcode with a shared lookup
...
\appendix
\input{sections/033_executor}
\input{sections/appendix_isa}
\input{sections/appendix_units}
\input{sections/appendix_levers}
\input{sections/appendix_ipcore}
