% Ziren as a processor block diagram: chips (chips) and buses (interactions).
\begin{tikzpicture}[
  every node/.style={font=\small},
  chip/.style={rectangle, draw=zirenblue!85, rounded corners=2pt, text width=2.0cm, minimum height=0.7cm, align=center, fill=zirenblue!7, inner sep=2pt},
  accel/.style={chip, draw=zirenorange!90, fill=zirenorange!10},
  ...
  % instruction chips (functional chips), pitch 2.3 cm
  \node[chip] (alu)   at (0,0)    {ALU\\\texttt{AddSub}, \texttt{Bitwise}};
  \node[chip] (shift) at (2.3,0)  {Shift\\\texttt{ShiftLeft}\\\texttt{(Imm)}};
  \node[chip] (mul)   at (4.6,0)  {Mul/Div\\\texttt{Mul}, \texttt{DivRem}};
  \node[chip] (ctl)   at (6.9,0)  {Control\\\texttt{Branch}, \texttt{Jump}};
  \node[chip] (ld)    at (9.2,0)  {Load/Store\\\texttt{LoadWord}, \ldots};
  \node[chip] (sys)   at (11.5,0) {Syscall\\\texttt{Syscall}\\\texttt{Instrs}};
